...
class ProjectView(ProjectViewTemplate):
  def __init__(self, **properties):
    ...
    self.editable_project_name.add_event_handler(
      "x-change-text", self.change_project_name
    )
    self.get_project_and_columns()

    # self.add_new_column_link = Link(
    #   text="New Column", icon="fa:plus", spacing_above="small", spacing_below="none"
    # )
  ...
  def add_data_to_grid(self, **event_args):
    ...
    self.get_project_and_columns()
    cols = [
      {"id": row.get_id(), "title": row["title"], "data_key": row.get_id()}
  ...
  def create_header_link(self, row):
    ...
    flow_panel = FlowPanel(
      role="visible-hover-only", spacing_above="none", spacing_below="none"
    )
    editable_link = EditableLink(
      text=row["title"], column_id=row.get_id(), type="header"
    )
